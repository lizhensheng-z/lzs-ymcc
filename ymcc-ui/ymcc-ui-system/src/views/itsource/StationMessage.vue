<template>
	<section>
		...
		<el-table :data="tableData" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="isread" label="已读状态" :formatter="formatIsread" sortable>
			</el-table-column>
		...
		<el-dialog :title="dialogTitle" :visible.sync="addFormVisible" :close-on-click-modal="false">
			<el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
				...
				<el-form-item label="类型" prop="type">
					<el-input v-model="addForm.type" auto-complete="off" placeholder="请输入类型"></el-input>
				</el-form-item>
				<el-form-item label="已读状态" prop="isread">
					<el-select v-model="addForm.isread" placeholder="请选择状态">
		...
		methods: {
			...
			del: function(row) {
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					this.listLoading = true;
					this.$http.delete("/common/messageStation/" + row.id).then(result => {
						let {
							success,
							data,
							message,
				...
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							this.$http.post("/common/messageStation/save", this.addForm).then(result => {
								let {
									success,
									data,
									message,
			...
			batchRemove: function() {
				...
				this.$confirm('确认删除选中记录吗？', '提示', {
					...
					this.listLoading = true;
					let deletePromises = ids.map(id => this.$http.delete("/common/messageStation/" + id));
