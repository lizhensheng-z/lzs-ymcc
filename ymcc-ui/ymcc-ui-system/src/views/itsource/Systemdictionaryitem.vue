<template>
	<section>
		...
		<el-table :data="tableData" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="parentId" label="所属字典" sortable>
			</el-table-column>
			<el-table-column prop="name" label="名称" sortable>
			</el-table-column>
			<el-table-column prop="requence" label="排序" sortable>
			</el-table-column>
		...
		<el-dialog :title="dialogTitle" :visible.sync="addFormVisible" :close-on-click-modal="false">
			<el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
				<el-form-item label="所属字典" prop="parentId">
					<el-input v-model.number="addForm.parentId" auto-complete="off" placeholder="请输入所属字典ID"></el-input>
				</el-form-item>
				<el-form-item label="名称" prop="name">
					<el-input v-model="addForm.name" auto-complete="off" placeholder="请输入名称"></el-input>
				</el-form-item>
				<el-form-item label="排序" prop="requence">
					<el-input v-model.number="addForm.requence" auto-complete="off" placeholder="请输入排序号"></el-input>
				</el-form-item>
				<el-form-item label="简介" prop="intro">
					<el-input v-model="addForm.intro" type="textarea" :rows="3" auto-complete="off" placeholder="请输入简介"></el-input>
				</el-form-item>
			</el-form>
	...
	export default {
		data() {
			return {
				...
				addFormRules: {
					parentId: [
						{ required: true, message: '请输入所属字典ID', trigger: 'blur' },
						{ type: 'number', message: '所属字典ID必须为数字', trigger: 'blur' }
					],
					name: [
		...
		methods: {
			...
			del: function(row) {
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					this.listLoading = true;
					this.$http.delete("/system/systemdictionaryitem/" + row.id).then(result => {
						let {
							success,
							data,
							message,
			...
			batchRemove: function() {
				...
				this.$confirm('确认删除选中记录吗？', '提示', {
					...
					this.listLoading = true;
					let deletePromises = ids.map(id => this.$http.delete("/system/systemdictionaryitem/" + id));
