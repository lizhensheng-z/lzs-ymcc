		...
		<el-dialog :title="dialogTitle" :visible.sync="addFormVisible" :close-on-click-modal="false">
			<el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
				<el-form-item label="编号" prop="sn">
					<el-input v-model="addForm.sn" auto-complete="off" placeholder="请输入编号"></el-input>
				</el-form-item>
				<el-form-item label="标题" prop="name">
					<el-input v-model="addForm.name" auto-complete="off" placeholder="请输入标题"></el-input>
				</el-form-item>
		...
		methods: {
			...
			del: function(row) {
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					this.listLoading = true;
					this.$http.delete("/system/systemdictionary/" + row.id).then(result => {
						let {
							success,
							data,
							message,
				...
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							this.$http.post("/system/systemdictionary/save", this.addForm).then(result => {
								let {
									success,
									data,
									message,
			...
			batchRemove: function() {
				...
				this.$confirm('确认删除选中记录吗？', '提示', {
					...
					this.listLoading = true;
					let deletePromises = ids.map(id => this.$http.delete("/system/systemdictionary/" + id));
