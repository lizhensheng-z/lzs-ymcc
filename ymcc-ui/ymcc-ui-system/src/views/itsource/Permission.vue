<template>
	<section>
		...
		<el-table :data="tableData" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="sn" label="表达式" sortable>
			</el-table-column>
		...
		<el-dialog :title="dialogTitle" :visible.sync="addFormVisible" :close-on-click-modal="false">
			<el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
				<el-form-item label="权限名称" prop="name">
					<el-input v-model="addForm.name" auto-complete="off" placeholder="请输入权限名称"></el-input>
				</el-form-item>
				<el-form-item label="资源" prop="resource">
					<el-input v-model="addForm.resource" auto-complete="off" placeholder="请输入资源"></el-input>
				</el-form-item>
				<el-form-item label="表达式" prop="sn">
					<el-input v-model="addForm.sn" auto-complete="off" placeholder="请输入表达式"></el-input>
				</el-form-item>
				<el-form-item label="菜单ID" prop="menuId">
					<el-input v-model.number="addForm.menuId" auto-complete="off" placeholder="请输入菜单ID"></el-input>
				</el-form-item>
		...
		methods: {
			...
			del: function(row) {
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					this.listLoading = true;
					this.$http.delete("/uaa/permission/" + row.id).then(result => {
						let {
							success,
							data,
							message,
				...
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							this.$http.post("/uaa/permission/save", this.addForm).then(result => {
								let {
									success,
									data,
									message,
			...
			batchRemove: function() {
				...
				this.$confirm('确认删除选中记录吗？', '提示', {
					...
					this.listLoading = true;
					let deletePromises = ids.map(id => this.$http.delete("/uaa/permission/" + id));
